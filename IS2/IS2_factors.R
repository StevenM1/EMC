...
IS2_factor <- function(samples, filter, IS_samples = 250, n_particles = 250, n_cores = 1, df = 3){
  data <- samples$data
  ...
  # grab the samples stage of PMwG
  ...
  constraintMat <- attributes(samples)$constraintMat
  ...
  prior_dist = function(parameters, priors = samples$prior, n_randeffect){ ###mod notes: the samples$prior needs to be fixed/passed in some other time
  ...
  compute_lw=function(prop_theta,data,n_subjects,n_particles,n_randeffect,mu_tilde,sigma_tilde,i, prior_dist=prior_dist, samples=samples, constraintMat, n_factors){
    ...
    #do equation 10
    logw_num <- logp.out+prior_dist(parameters = prop_theta[i,], priors = samples$prior, n_randeffect)
  ...
  if (n_cores>1){
    tmp <- mclapply(X=1:IS_samples,mc.cores = n_cores, FUN = compute_lw, prop_theta = prop_theta,data = data,n_subjects= n_subjects,n_particles = n_particles,
                    n_randeffect = n_randeffect,mu_tilde=mu_tilde,sigma_tilde = sigma_tilde, prior_dist=prior_dist, samples=samples, constraintMat, n_factors)
  } else{
    for (i in 1:IS_samples){
      cat(i)
      # save.image("tmpbug10.RData")
      tmp[i]<-compute_lw(prop_theta, data, n_subjects, n_particles, n_randeffect, mu_tilde, sigma_tilde, i, prior_dist=prior_dist, samples=samples, constraintMat, n_factors)
    }
  }
