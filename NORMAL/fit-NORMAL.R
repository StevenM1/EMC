...
designNORMAL <- make_design(Flist=list(mean ~ S, sd ~ 1),
  Ffactors=list(subjects=1,S=1:2),Rlevels=1,model=normal)
...
par(mfrow=c(1,3))
profile_pmwg(pname="mean",p=p_vector,p_min=-.5,p_max=.5,dadm=dadmNORMAL)
profile_pmwg(pname="mean_S2",p=p_vector,p_min=0,p_max=2,dadm=dadmNORMAL)
profile_pmwg(pname="sd",p=p_vector,p_min=log(.5),p_max=log(1.5),dadm=dadmNORMAL)
