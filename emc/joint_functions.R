...
single_out_joint <- function(joint_samples, i){
  par_names <- joint_samples$par_names
  prefix <- unique(gsub("[-].*", "", par_names))[i]
  current_pars <- par_names[grep(paste0(prefix, "-"), par_names)]
  replacement <-  gsub(".*[-]", "", current_pars)
  single_samples <- joint_samples
  single_samples$samples <- base::rapply(joint_samples$samples, f = function(x) get_single(x, prefix, current_pars, replacement), how = "replace")
...
get_single <- function(object, prefix, current_pars, replacement){
