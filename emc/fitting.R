  ...
  repeat {
    ...
    repeat {
      samplers_new <- mclapply(samplers,run_stage,stage=stage,iter=iter, mix=mix,
        ...
        pdist_update_n=pdist_update_n,n_cores=cores_per_chain,mc.cores=cores_for_chains,
        min_unique=min_unique,epsilon_upper_bound=epsilon_upper_bound)
      if (class(samplers_new)=="try-error" || 
        any(lapply(samplers_new,class)=="try-error")) {
    ...
    if (!check_adapt(samplers,verbose=FALSE)) {
      ...
      repeat {
        samplers_new <- mclapply(samplers,run_stages,iter=c(NA,nadapt,NA),
          ...
          pdist_update_n=pdist_update_n,min_unique=min_unique,
          mc.cores=cores_for_chains)
        if (class(samplers_new)=="try-error" || any(lapply(samplers_new,class)=="try-error")) {
          save(samplers_new,samplers,nadapt,particles,particle_factor,p_accept,pdist_update_n,
              min_unique,epsilon_upper_bound,file="fail_run_stage.RData")
          run_try <- run_try + 1
