...
gibbs_step_blocked <- function(sampler){
  ...
  for(group in 1:sampler$n_par_groups){
    ...
    if(n_pars > 1){
      ...
      mean_mu <- as.vector(var_mu %*% (last$tvinv[group_idx, group_idx] %*% apply(last$alpha[group_idx,], 1, sum) +
      ...
      theta_temp <- last$alpha[group_idx,] - tmu
      ...
      mean_mu = var_mu * (sum(last$alpha[group_idx,]) * last$tvinv[group_idx] + prior$theta_mu_invar[group_idx, group_idx] * prior$theta_mu_mean[group_idx])
  ...
  return(list(tmu = tmu_out,tvar = tvar_out,tvinv = tvinv_out,a_half = a_half_out,alpha = last$alpha))
}
